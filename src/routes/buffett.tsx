...
import { evaluateBuffettResult, evaluateModifiedBuffettResult } from '../features/buffett/buffett-gates'
import { BUFFETT_THRESHOLD, MODIFIED_BUFFETT_THRESHOLD } from '../features/buffett/types'
import { LoadingState } from '../components/shared/loading-state'
import { BuffettTable } from '../components/features/buffett'
import { RefreshCw } from 'lucide-react'
import type { BuffettPageRow } from '../features/buffett'

const REFRESH_CHUNK_SIZE = 200

type BuffettMode = 'original' | 'modified'

const MODE_META: Record<BuffettMode, { label: string; pageTitle: string; threshold: number; gates: number }> = {
  original: { label: 'Original', pageTitle: 'Warren Buffett Stocks', threshold: BUFFETT_THRESHOLD, gates: 9 },
  modified: { label: 'Modified', pageTitle: 'Warren Buffett Modified Stocks', threshold: MODIFIED_BUFFETT_THRESHOLD, gates: 9 },
}

export function BuffettPage() {
  const [loading, setLoading] = useState(true)
  const [refreshing, setRefreshing] = useState(false)
  const [rows, setRows] = useState<BuffettPageRow[]>([])
  ...
  const [mode, setMode] = useState<BuffettMode>('original')
  const abortRef = useRef<AbortController | null>(null)

  const meta = MODE_META[mode]

  const buildRow = useCallback((s: { symbol: string; name: string; sector: string; marketCap?: number; lastPrice?: number }, f: any, currentMode: BuffettMode) => {
    const evaluate = currentMode === 'modified' ? evaluateModifiedBuffettResult : evaluateBuffettResult
    const buffett = evaluate(f ?? {})
    ...
    return {
      ...
      roe: f?.roe,
      score: undefined,
      gatesPassed: buffett.gatesPassed,
      gatesTotal: buffett.gatesTotal,
      isBuffettCompliant: buffett.isBuffettCompliant,
      buffettScore: buffett.buffettScore,
      gateResults: buffett.gateResults,
      _hasData: hasData,
    }
  }, [])

  const loadData = useCallback(async (currentMode: BuffettMode) => {
    setLoading(true)
    await ensureSeeded()
    ...
    const compliant = all.filter((r) => r.isBuffettCompliant).sort((a, b) => b.gatesPassed - a.gatesPassed || b.buffettScore - a.buffettScore)
      ...
      setRows((prev) => {
        ...
        for (const symbol of topSymbols) {
          ...
          if (result?.data) {
            ...
            if (existingIdx >= 0) {
              if (row.isBuffettCompliant) {
                updated[existingIdx] = row
              } else {
                updated.splice(existingIdx, 1)
              }
            } else if (row.isBuffettCompliant) {
              updated.push(row)
            }
          }
        }

        updated.sort((a, b) => b.gatesPassed - a.gatesPassed || b.buffettScore - a.buffettScore)
        return updated
      })
  ...
  const handleModeSwitch = (newMode: BuffettMode) => {
    setMode(newMode)
    setPage(1)
  }

  const topStocks = useMemo(() => rows.filter((r) => r.gatesPassed >= 8), [rows])
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--muted-foreground)]">
            Stocks that pass at least {meta.threshold} of {meta.gates} criteria from Warren Buffett's investment philosophy{mode === 'modified' ? ' (adapted for Indian markets)' : ''}.
          </p>
        </div>
        <button
      ...
      <div className="mb-6 rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        <div className="grid grid-cols-4 gap-4 text-center">
          ...
          <div>
            ...
            <div className="text-xs text-[var(--muted-foreground)]">Buffett-Compliant</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-[var(--score-amber)]">{topStocks.length}</div>
            <div className="text-xs text-[var(--muted-foreground)]">Top Picks (8-9)</div>
          </div>
        </div>
      </div>
      ...
      {topStocks.length > 0 && (
        <div className="mb-6 rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          <h2 className="mb-2 text-sm font-semibold text-[var(--foreground)]">
            Top {mode === 'modified' ? 'Modified ' : ''}Buffett Picks ({topStocks.length} stocks pass 8-9 gates)
          </h2>
          <div className="flex flex-wrap gap-2">
            {topStocks.map((s) => (
              ...
              >
                {s.symbol} <span className="opacity-70">({s.gatesPassed}/9)</span>
              </span>
            ))}
          </div>
        </div>
      ...
      {dataAvailable > 0 && rows.length === 0 && (
        <div className="mb-6 rounded-lg border border-[var(--border)] bg-[var(--card)] p-6 text-center">
          <p className="text-sm text-[var(--muted-foreground)]">
            {dataAvailable} stocks have fundamental data, but none currently pass all {meta.threshold} of {meta.gates} Buffett{mode === 'modified' ? ' Modified' : ''} criteria.
            {mode === 'original' ? " The Indian market tends to have higher P/E ratios and leverage than Buffett's thresholds." : ' Try switching to Original mode for stricter criteria.'}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
        <div className="lg:col-span-3">
          <div aria-live="polite" aria-atomic="true" className="sr-only">
            {rows.length} Buffett-compliant stocks found
          </div>
          <BuffettTable rows={rows} page={page} onPageChange={setPage} />
        </div>

        <aside className="space-y-6">
          <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
            <h3 className="mb-3 text-sm font-semibold text-[var(--foreground)]">
              Buffett's 4-Filter Approach
            </h3>
            <ol className="space-y-3 text-xs text-[var(--muted-foreground)]">
              <li className="flex items-start gap-2">
                ...
                <span><strong className="text-[var(--foreground)]">Understandable Business</strong> — within your circle of competence</span>
              </li>
              <li className="flex items-start gap-2">
                ...
                <span><strong className="text-[var(--foreground)]">Favorable Long-Term Economics</strong> — wide and durable competitive moat</span>
              </li>
              <li className="flex items-start gap-2">
                ...
                <span><strong className="text-[var(--foreground)]">Able & Trustworthy Management</strong> — integrity and rational capital allocation</span>
              </li>
              <li className="flex items-start gap-2">
                ...
                <span><strong className="text-[var(--foreground)]">Sensible Price</strong> — margin of safety below intrinsic value</span>
              </li>
            </ol>
          </div>
            ...
            <div className="space-y-1.5 text-xs">
              {mode === 'original' ? (
                <>
                  ...
                  <div className="flex justify-between">
                    <span className="text-[var(--muted-foreground)]">Op. Margin</span>
                    <span className="font-medium text-[var(--foreground)]">≥ 10%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--muted-foreground)]">Net Margin</span>
                    <span className="font-medium text-[var(--foreground)]">≥ 8%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--muted-foreground)]">FCF</span>
                    <span className="font-medium text-[var(--foreground)]">&gt; 0</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--muted-foreground)]">Market Cap</span>
                    <span className="font-medium text-[var(--foreground)]">&gt; ₹4,000 Cr</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--muted-foreground)]">Promoter</span>
                    <span className="font-medium text-[var(--foreground)]">≥ 25%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--muted-foreground)]">Pledged</span>
                    <span className="font-medium text-[var(--foreground)]">= 0%</span>
                  </div>
                </>
              ) : (
          ...
          <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
            ...
            <p className="mb-3 text-xs text-[var(--muted-foreground)]">
              The Buffett Score re-weights categories to match his priorities:
            </p>
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between">
                <span className="text-[var(--muted-foreground)]">Quality (ROE, margins)</span>
              ...
              <div className="flex justify-between">
                <span className="text-[var(--muted-foreground)]">Ownership</span>
                <span className="font-medium text-[var(--foreground)]">5%</span>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
            ...
            <ul className="space-y-1.5 text-[11px] text-[var(--muted-foreground)]">
              <li>• <em>The Essays of Warren Buffett</em> (Cunningham)</li>
              <li>• <em>Buffettology</em> (Buffett & Clark)</li>
              <li>• <em>The Warren Buffett Way</em> (Hagstrom)</li>
              <li>• Berkshire Hathaway Annual Letters</li>
            </ul>
          </div>
        </aside>
      </div>
