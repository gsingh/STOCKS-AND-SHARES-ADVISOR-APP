...
type SortKey = keyof BuffettPageRow
type SortDir = 'asc' | 'desc'

interface BuffettTableProps {
  rows: BuffettPageRow[]
  page: number
  onPageChange: (page: number) => void
}

function getBuffettGateBadge(gatesPassed: number, total: number) {
...
const SORTABLE_COLUMNS: { key: SortKey; label: string; align?: string }[] = [
  ...
  { key: 'buffettScore', label: 'B-Score', align: 'text-right' },
]

export function BuffettTable({ rows, page, onPageChange }: BuffettTableProps) {
  ...
  const toggleSort = useCallback((key: SortKey) => {
    setSortKey((prev) => {
      ...
      setSortDir(key === 'gatesPassed' || key === 'buffettScore' ? 'desc' : 'asc')
      return key
    })
  }, [])
  ...
  return (
    <div>
      <div className="overflow-x-auto rounded-lg border border-[var(--border)]" role="region" aria-label="Buffett-compliant stocks">
                ...
                >
                  <td className="px-2 py-3">
                    <button
                      ...
                      aria-label={`${expanded === row.symbol ? 'Hide' : 'Show'} Buffett criteria for ${row.symbol}`}
                    >
                      {expanded === row.symbol ? (
                        <ChevronUp size={16} />
                      ) : (
                  ...
                  <td className="px-4 py-3 text-right">
                    {getBuffettGateBadge(row.gatesPassed, row.gatesTotal)}
                  </td>
                  <td className="px-4 py-3 text-right">
            ...
            {paginated.length === 0 && (
              <tr>
                <td colSpan={10} className="px-4 py-12 text-center text-sm text-[var(--muted-foreground)]">
                  No Buffett-compliant stocks found. Try refreshing data for more stocks.
                </td>
              </tr>
            )}
          </tbody>
