...
type SortKey = keyof EnterprisingPageRow
type SortDir = 'asc' | 'desc'

interface EnterprisingTableProps {
  rows: EnterprisingPageRow[]
  page: number
  onPageChange: (page: number) => void
}

function getEnterprisingGateBadge(gatesPassed: number, total: number) {
...
const SORTABLE_COLUMNS: { key: SortKey; label: string; align?: string }[] = [
  ...
  { key: 'enterprisingScore', label: 'E-Score', align: 'text-right' },
]

export function EnterprisingTable({ rows, page, onPageChange }: EnterprisingTableProps) {
  ...
  const toggleSort = useCallback((key: SortKey) => {
    if (sortKey === key) {
      ...
      setSortKey(key)
      setSortDir(key === 'gatesPassed' || key === 'enterprisingScore' ? 'desc' : 'asc')
    }
  }, [sortKey])
  ...
  return (
    <div>
      <div className="overflow-x-auto rounded-lg border border-[var(--border)]" role="region" aria-label="Enterprising Investor compliant stocks">
                ...
                >
                  <td className="px-2 py-3">
                    <button
                      ...
                      aria-label={`${expanded === row.symbol ? 'Hide' : 'Show'} Enterprising criteria for ${row.symbol}`}
                    >
                      {expanded === row.symbol ? (
                        <ChevronUp size={16} />
                      ) : (
                  ...
                  <td className="px-4 py-3 text-right">
                    {getEnterprisingGateBadge(row.gatesPassed, row.gatesTotal)}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <span className="inline-flex items-center rounded-full bg-[var(--score-green-bg)] px-2 py-0.5 text-xs font-semibold text-[var(--score-green)]">
                      {row.enterprisingScore.toFixed(0)}
                    </span>
                  </td>
                </tr>
                {expanded === row.symbol && (
            ...
            {paginated.length === 0 && (
              <tr>
                <td colSpan={10} className="px-4 py-12 text-center text-sm text-[var(--muted-foreground)]">
                  No Enterprising Investor-compliant stocks found. Try refreshing data for more stocks.
                </td>
              </tr>
            )}
          </tbody>
