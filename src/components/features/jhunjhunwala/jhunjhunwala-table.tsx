...
type SortKey = keyof JhunjhunwalaPageRow
type SortDir = 'asc' | 'desc'

interface JhunjhunwalaTableProps {
  rows: JhunjhunwalaPageRow[]
  page: number
  onPageChange: (page: number) => void
}

function getJhunjhunwalaGateBadge(gatesPassed: number, total: number) {
  const ratio = gatesPassed / total
  const cls = ratio >= 0.88
...
const SORTABLE_COLUMNS: { key: SortKey; label: string; align?: string }[] = [
  ...
  { key: 'jhunjhunwalaScore', label: 'J-Score', align: 'text-right' },
]

export function JhunjhunwalaTable({ rows, page, onPageChange }: JhunjhunwalaTableProps) {
  ...
  const toggleSort = useCallback((key: SortKey) => {
    setSortKey((prev) => {
      ...
      setSortDir(key === 'gatesPassed' || key === 'jhunjhunwalaScore' ? 'desc' : 'asc')
      return key
    })
  }, [])
  ...
  return (
    <div>
      <div className="overflow-x-auto rounded-lg border border-[var(--border)]" role="region" aria-label="Jhunjhunwala-compliant stocks">
                ...
                >
                  <td className="px-2 py-3">
                    <button
                      ...
                      aria-label={`${expanded === row.symbol ? 'Hide' : 'Show'} Jhunjhunwala criteria for ${row.symbol}`}
                    >
                      {expanded === row.symbol ? (
                        <ChevronUp size={16} />
                      ) : (
                  ...
                  <td className="px-4 py-3 text-right">
                    {getJhunjhunwalaGateBadge(row.gatesPassed, row.gatesTotal)}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <span className="inline-flex items-center rounded-full bg-[var(--score-green-bg)] px-2 py-0.5 text-xs font-semibold text-[var(--score-green)]">
                      {row.jhunjhunwalaScore.toFixed(0)}
                    </span>
                  </td>
                </tr>
                {expanded === row.symbol && (
                  <tr key={`${row.symbol}-expanded`}>
                    <td />
                    <td colSpan={11} className="px-4 py-3 bg-[var(--muted)]/50">
                      <GateChecklist gateResults={row.gateResults} />
                    </td>
                  </tr>
                )}
              </>
            ...
            {paginated.length === 0 && (
              <tr>
                <td colSpan={12} className="px-4 py-12 text-center text-sm text-[var(--muted-foreground)]">
                  No Jhunjhunwala-compliant stocks found. Try refreshing data for more stocks.
                </td>
              </tr>
            )}
          </tbody>
